...
- **匯出給 AI 分析** — 把原始粒度的資料匯成一組 CSV，附資料字典與已知陷阱，可以直接丟給 AI 做營運分析
...
**區間收在「App Store 最新已結算的那一天」，不是昨天，更不是今天。** 今天除了 AdMob 之外全都是 0（Apple 慢一到兩天、Play 慢三到七天），含進來等於每個區間都摻一天殘缺的資料 ——「1 天」最明顯，但 30 天、90 天的總計與日均也一樣被拉低，只是不明顯，那更糟。所以結束點跟著資料走，被往回拉的時候畫面上會說明原因。今天有自己的位置：「最新單日數據」與總收入的「今天」格，都標著尚未結算。
...
**設定備份** — 把設定與三個來源的憑證打包成一個檔案，換機或重灌時匯入就好，不必重新授權。因為它含 App Store Connect 的私鑰與 OAuth token，所以**一律用你自己設的密碼加密**（scrypt + AES-256-GCM）。密碼不存在任何地方，忘了只能重新授權。檔頭刻意不含任何識別資訊 —— 檔案外流時，在解開之前看不出它屬於誰。

設定另外每次變更前自動留一份快照（保留 10 版）。設定檔萬一損毀，會從最新的好快照還原，而不是安靜地套用預設值。

**報表資料庫備份** — 每次更新成功後檢查，超過 7 天就自動備份一次，保留 4 份；也可以手動備份或另存他處。

> 這不是「以防萬一」。廣告與 Play 的數字重抓就有，但 **Apple 的銷售報表只保留 365 天** —— 超過一年的那一段，你的資料庫是它唯一的存在，而那個區段每過一天就長一天。
...
**某個來源突然變成「未設定」** — 先看畫面上寫的路徑。憑證檔如果放在外接硬碟或雲端硬碟（pCloud、OneDrive、iCloud），那顆沒掛上的時候檔案就讀不到。AppScope 會直接告訴你是哪一層資料夾不存在，並且說明設定沒有遺失 —— 把磁碟掛回來就會恢復。
